...
namespace MFaaP.MFWSClient
{
	public partial class MFWSClient
	{
		...
		public async Task<byte[]> DownloadFile(int objectType, int objectId, int fileId, int? objectVersion = null)
		{
			...
			// Execute the request.
			var response = await this.Get(request);
		...
		/// <param name="objectVersion">The verison of the object, or null for the latest.</param>
		/// <returns>The raw response from the HTTP request.</returns>
		public async Task DownloadFile(int objectType, int objectId, int fileId, System.IO.Stream outputStream, int? objectVersion = null)
		{
			// Create the version string to be used for the uri segment.
			var versionString = objectVersion?.ToString() ?? "latest";
		...
		public Task DownloadFile(int objectType, int objectId, int fileId, string outputFileName, int? objectVersion = null)
		{
			...
			return this.DownloadFile(objectType, objectId, fileId, outputFileInfo, objectVersion);
		}
		...
		public async Task DownloadFile(int objectType, int objectId, int fileId, System.IO.FileInfo outputFileInfo, int? objectVersion = null)
		{
			...
			using (var stream = outputFileInfo.Create())
			{
				...
				await this.DownloadFile(objectType, objectId, fileId, stream, objectVersion);
			}
		}
	}
}
